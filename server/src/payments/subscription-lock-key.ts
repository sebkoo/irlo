 ...
 * `consumePurchaseEvent` today, and `consumeSubscriptionEconomicEvent`
 * (renewed/refunded — not yet built as of this commit) once it lands. A
 * single source of truth so the two independently-triggered functions
 * serialize against each other on the *same* lock, not two similar-looking-
 * but-different strings. Test harnesses that pre-hold this lock
